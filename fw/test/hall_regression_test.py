...
# The 20260914-synth-* datasets are generated by hall_synthetic_data.py
# (deterministic; see that file for the model).  The real captures are
# all default-polarity motors with mild accelerations; these cover an
# inverted hall line -- where the late, pull-up limited rising edges
# no longer alternate with the sharp falling ones -- and hard
# accelerations that repeatedly cross the slow/PLL boundary.
#
# The acceleration datasets are not run at 50 Hz: a PLL that slow
# cannot follow their 17000 edges/s^2 ramps and the flat-window bias
# detector, which only checks that the true speed is flat, would read
# the resulting lag as a bias.
#
# aux.hall.polarity setting each dataset was captured with (default 0).
HALL_POLARITY = {
    '20260914-synth-polarity-inverted-a': 1,
}
...
TESTS = [
    ...
    ('20260914-synth-polarity-inverted-a',  50,  0.0142,     0.6,   502.0,   239.0,     1.0,    2.99),
    ...
    ('20260914-synth-polarity-inverted-a', 124,  0.0075,     0.6,   521.0,   239.0,     1.0,    2.99),
    ('20260914-synth-accel-trapezoid', 124,  0.0223,    1.04,  8500.0,  1160.0,     1.0,   0.343),
    ('20260914-synth-accel-sine',     124,  0.0304,    1.16,  8890.0,  1240.0,     1.0,     0.3),
    ...
    ('20260914-synth-polarity-inverted-a', 248, 0.00717,     0.6,   775.0,   239.0,     1.0,    2.99),
    ('20260914-synth-accel-trapezoid', 248, 0.00998,   0.773,  4210.0,   857.0,    1.01,   0.343),
    ('20260914-synth-accel-sine',     248,  0.0118,   0.674,  4480.0,  1330.0,     1.0,     0.3),
    ...
    ('20260914-synth-polarity-inverted-a', 496,  0.0148,     0.6,  8560.0,   343.0,     1.0,    2.99),
    ('20260914-synth-accel-trapezoid', 496,  0.0137,   0.637,  5920.0,   601.0,    1.01,   0.343),
    ('20260914-synth-accel-sine',     496,  0.0146,     0.6, 14100.0,  1260.0,     1.0,     0.3),
    ...
    ('20260914-synth-polarity-inverted-a', 992,  0.0148,     0.6,  8560.0,   343.0,     1.0,    2.99),
    ('20260914-synth-accel-trapezoid', 992,  0.0164,   0.637,  2530.0,   379.0,    1.01,   0.343),
    ('20260914-synth-accel-sine',     992,  0.0157,     0.6,  2680.0,  1260.0,     1.0,     0.3),
]
...
def main():
    ...
    for test_name, pll_filter_hz, pmetric, pmax, vmetric, vmax, vbias, vripple in TESTS:
        ...
        hall_polarity = HALL_POLARITY.get(test_name, 0)
        result_json = subprocess.check_output([args.binary, '--input', os.path.join(tdir.name, f"{test_name}.dat"), "--pll_filter_hz", f"{pll_filter_hz}", "--rate_hz", f"{rate_hz}", "--hall_polarity", f"{hall_polarity}"])
